...
describe("calorie target", () => {
  it("applies activity then a conservative deficit", () => {
    const t = calorieTarget(profile({}), 2026)!;
    // BMR 1780 × 1.55 = 2759; deficit min(500, 413.85) = 413.85
    expect(t.tdee).toBe(2759);
    expect(t.kcal).toBe(2345);
    expect(t.floorApplied).toBe(false);
  });

  it("caps the deficit at 500 kcal for high TDEEs", () => {
    const t = calorieTarget(profile({ weightKg: 110, activity: "high" }), 2026)!;
    expect(t.tdee - t.kcal).toBeLessThanOrEqual(500);
  });

  it("NEVER goes below the hard floor", () => {
    const t = calorieTarget(
      profile({ sex: "female", weightKg: 45, heightCm: 150, birthYear: 1956, activity: "sedentary" }),
      2026,
    )!;
